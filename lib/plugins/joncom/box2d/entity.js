    ...
    ig.Entity.inject({
        ...
        init: function(x, y, settings) {
            ...
            if (!ig.global.wm && this.hasBody) {
                ...
                this.uniFriction = this.friction.x;
				var friction = this.uniFriction;
				
                this.createBody(friction, settings);
                if(this.mass !== null) {
        ...
        update: function() {
			//body not destroyed, try to destroy it outside of a callback
			if (this._killed && this.body) {
				this.kill();
			}
		...
		//altered by substandardgaussian to set filter data
        createBody: function(friction, settings) {
			...
			fixtureDef.userData = {name: 'normal', categoryBits: settings.categoryBits, maskBits: settings.maskBits};
			if (settings) {
				fixtureDef.filter.categoryBits = settings.categoryBits;
				fixtureDef.filter.maskBits = settings.maskBits;
			}
		...
		//added by substandardgaussian to provoke engine response when otherwise short-circuiting physics
		//call immediately after directly modifying a body's position or velocity to wake it up to respond to those changes
		//alternatively, simply set the body's wake status to true... pretty sure this doesn't affect its ability to sleep later, which has a different setter function
		jiggle: function() {
			this.body.ApplyForce(new Box2D.Common.Math.b2Vec2(0, 0), this.body.GetPosition());
		}
